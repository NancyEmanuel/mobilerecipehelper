...
// Represents the entire JSON response from TheMealDB API
data class MealDbResponse(
    // "meals" is the list returned by the API. It may be null if no results.
    @SerializedName("meals") val meals: List<Meal>?
)

// Represents a single meal object from TheMealDB API
data class Meal(
    ...
    // Up to 20 ingredient fields (API limitation).
    // Some may be null or empty depending on the recipe.
    ...
    // Matching measurement fields ("1 cup", "2 tbsp", etc.)
